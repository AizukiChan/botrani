        ...
        if (button.id == 'createTicket') {
            ...
            if (!button.guild.channels.cache.find(x => x.name === ticketName)) {
                ...
                let TicketError = new MessageEmbed()
                    .setColor("0xFF0000")
                    .setDescription(`Anda Telah Membuat Tiket Sebelumnya ⛔`)
                    .setTitle(`**❌ | Error**`)
            ...
            button.channel.send({
                embed: {
                    description: `Tiket Telah Ditutup Oleh <@!${button.clicker.user.id}>🙃`,
                    color: 'RANDOM'
                }
            })
            let btn = new MessageButton()
            ...
            button.channel.send({
                embed: {
                    description: 'Klik **"⛔"** Untuk Menghapus Tiket dan Klik **"🔓"** Untuk Membuka Kembali Tiket',
                    color: 'RED'
                },
                component: row
            })
            ...
            button.channel.send({
                embed: {
                    description: `${button.clicker.user}Tiket Akan Ditutup dalam` + " `5` " + `Detik Sabar😁`,
                    color: 'RANDOM'
                }
            });
...
\`${button.channel.name}\`\n**Dar Tarikhe :** <t:${TimeStampDate}:R>`);
            ...
            button.channel.send({
                embed: {
                    description: `Ticket Telah Dibuka Oleh <@!${button.clicker.user.id}>`,
                    color: 'RANDOM'
                }
            })
            var member = await db.get(`TicketControl_${button.channel.id}`);

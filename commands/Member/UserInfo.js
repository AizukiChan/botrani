module.exports = {
  ...
  category: 'Member 👻',
  utilisation: 'userinfo [name | mention | ID]',
    ...
    try {
      ...
      await axios.get(`https://discord.com/api/users/${Member.id}`, {
        ...
        let UserEmbed = new Discord.MessageEmbed()
          ...
          .addField(`✨| User Status:`, `**${statuses[Member.user.presence.status]} ${Member.user.presence.status}**`, true)
          .addField('🎬| User Activity:', `**${userstatus}**`, true)
          .addField('🏳| User Flags:', `**${userFlags.length ? userFlags.map(flag => flags[flag]).join(', ') : 'Gada🗿'}**`, true)
          .addField(`📅| Date of Join Discord:`, `**<t:${createdAtDate}:R>**`, true)
          .addField(`📈| Date of Join Server:`, `**<t:${joinedAtDate}:R>**`, true)
          .addField(`📸| [${memberRolesCount}] Roles:`, `**[${memberRoles}]**`, true)
        UserEmbed.addField(`🎨| Color of User Profile:`, `**${accent_color ? "#" + accent_color : "Gada🗿"}**`, true)
        UserEmbed.addField(`🎯| Highest Role In Server:`, `**<@&${Member.roles.highest.id}>**`, true)
        UserEmbed.addField('💻| User Permissions In Server:', `**${Member.permissions.toArray().map(p => `\`${p}\``).join(", ")}**`, true)
        UserEmbed.addField('🤖| User Is a Bot?:', `**\`${Member.user.bot === true ? "Yes✅" : "No❌"}\`**`, true)
